...
**Academic Outputs**:
The canonical public label for the proof page that contains the complete journal-paper record grouped by Research Family and subdirection, nested Paper Deep-Dive Pages, and an alternate Chronological Publication View. Use `学术成果 Academic Outputs` for the page title while preserving the existing `Publications.rst` filename and URL unless a redirect plan exists. On the page body, the Chronological Publication View should appear as a lightweight banner link to `PublicationsByYear.html`. In the RTD left sidebar, Paper Deep-Dive Pages should nest inside the Academic Outputs Research Family and subdirection hierarchy, not appear as direct children of Academic Outputs. The `期刊论文 Journal Papers` label is body copy, not a sidebar navigation node.
_Avoid_: 研究成果 Publications as a mixed-language page title, treating selected highlights as a mandatory Academic Outputs section, putting `Journal Papers` between Research Family and Paper Deep-Dive Page in the sidebar
...
**Paper Deep-Dive Pages**:
The journal-paper deep-dive pages under `docs/source/paper-notes/<publication_ref>.rst`. They keep stable `paper-notes/<publication_ref>.html` URLs and are registered in the Academic Outputs sidebar under Research Family and subdirection. The public Chinese label is `论文精解`; avoid using `论文解读` as the long-term public label for these RTD pages. The `期刊论文 Journal Papers` label should be emitted as an RST rubric in publication list pages so it remains body copy and does not interrupt the sidebar hierarchy.
