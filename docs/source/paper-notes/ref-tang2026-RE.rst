...
- 相比原始架构，所提出方法将训练时间缩短 :math:`32.92\%`，计算能耗降低 :math:`32.89\%`。
- Sparse Window-based Attention Block 在 :math:`s=2` 和 :math:`s=4` 时的显存占用约为 Swin-Transformer Block 的 :math:`29.37\%` 和 :math:`20.43\%`。
- 加入 Sparse Window-based Attention 后，参数量减少 :math:`19.50\%`，GPU 资源消耗降低约 :math:`70\%`。
