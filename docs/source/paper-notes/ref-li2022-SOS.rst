...
这篇论文比较同一座 Y 型半潜式浮式风机在混凝土平台和钢平台两种材料方案下的动力响应。 它重要，因为材料选择不仅改变建造和维护，也会改变重心、回复刚度、固有周期、塔底载荷和机舱加速度。 读者可以带走的结论是：钢平台和混凝土平台各有动力学优势，材料选型必须放到风浪耦合响应和目标海况中判断。

关键数字 / 关键结论卡
---------------------

- 研究对象是 :math:`5\,\mathrm{MW}` Y 型半潜式浮式风机，并通过 :math:`1{:}60` 比尺风浪联合模型试验和耦合仿真互相校验。
- 自由衰减试验显示，HSP3 钢平台的横摇/纵摇固有周期比 LCP3 混凝土平台约短 :math:`1.6\,\mathrm{s}`。
- 在额定运行风况下，HSP3 的平均平台纵摇角比 LCP3 小约 :math:`0.5^\circ`，但波频响应中仍可能出现相反趋势。
...
.. figure:: ../../../wechat/assets/public-safe/ref-li2022-SOS/fig-12-rao-comparisons.png
   :alt: 论文图 12 LCP3 与 HSP3 模型的响应幅值算子对比
   :align: center
   :width: 100%
   :class: paper-note-figure
...
.. figure:: ../../../wechat/assets/public-safe/ref-li2022-SOS/fig-26-structural-dynamics-statistics.png
   :alt: 论文图 26 组合随机风浪条件下风机结构动力学统计数据
   :align: center
   :width: 100%
   :class: paper-note-figure
