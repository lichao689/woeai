...
![图 1 建筑结构模型的结构图表示（不同颜色表示不同标准层）](../../assets/public-safe/ref-tang2025-JBE/fig-01-structural-graph.jpg)
...
有限元分析（FEA）方法通常计算量大且耗时，尤其是在涉及多次迭代的结构优化任务中。为提高效率，研究者已经开始使用神经网络预测结构响应。然而，工程数据集稀缺、高层建筑数据复杂，以及现有图神经网络（GNN）处理这类复杂数据时面临的挑战，限制了 GNN 代理模型的潜力。为解决这些问题，本文提出一种专门面向高层建筑结构的 GNN 训练与应用框架，目标是用 GNN 代理模型替代 FEA 完成结构分析。首先，本文用图表示高层建筑结构的完整信息；其次，通过参数化建模快速生成高层建筑结构数据集；最后，提出楼层特征增强策略，并修改损失函数以优化位移曲线模式。由此形成的高层建筑图神经网络 TBGNN，在较少数据、轻量化架构和较少计算资源条件下，在特定任务中表现良好。数值实验结果表明，改进模型在预测钢筋混凝土结构的结构位移、层间位移和一阶自振周期方面，相比其他 GNN 的准确率平均提高超过 $5\%$。此外，TBGNN 对风荷载和结构尺寸变化都非常敏感。与传统有限元分析方法相比，所提出的 GNN 代理模型节约约 $90\%$ 的时间。这些发现说明，用 GNN 代理模型服务高层建筑抗风结构设计优化具有可行性，并为这一重要挑战提供了有价值的思路。
...
高层建筑结构优化的难点之一，是每一次构件尺寸、楼层参数或荷载条件变化，都可能触发新一轮有限元建模与分析。在传统流程中，这些步骤可信但耗时，尤其不利于需要大量迭代的结构初步设计和优化探索。

图神经网络适合处理带拓扑关系的数据。建筑结构本身也天然包含节点、梁、柱、楼层和连接关系，因此可以被表示为图：

$$
G=(V,E,F)
$$

其中 $V$ 表示节点集合，$E$ 表示边或构件集合，$F$ 表示节点之间的邻接关系。本文的问题不是简单地把一个神经网络套到结构响应数据上，而是要回答：如何让图表示包含高层建筑的拓扑、构件、楼层、质量和风荷载信息，并让模型在风荷载变化和构件尺寸变化时仍能给出有工程意义的响应预测。
...
其中 $\beta_z$ 为风振系数，$\mu_s$ 为体型系数，$\mu_z$ 为风压高度变化系数，$w_0$ 为基本风压。这样，模型不是只学习几何形状，也能看到不同风压条件下的输入变化。

第二步是参数化建模与数据生成。论文通过 YJK-GAMA、自动分析程序、有限元分析和 PyTorch Geometric 数据转换，快速生成结构拓扑与结构响应数据。初始数据集包括 $998$ 个高层建筑结构，并通过改变基本风压 $w_0$ 扩展到 $2994$ 组；随后又生成面向超高层建筑的补充数据，使总数据集达到 $4194$ 个高层与超高层建筑结构。

![图 4 数据生成](../../assets/public-safe/ref-tang2025-JBE/fig-04-data-generation.jpg)
...
![图 6 TBGNN 架构](../../assets/public-safe/ref-tang2025-JBE/fig-06-tbgnn-architecture.jpg)
...
![图 8 面向超高层建筑的迁移学习](../../assets/public-safe/ref-tang2025-JBE/fig-08-transfer-learning.jpg)
...
论文首先在 $2944$ 组高层建筑结构数据上训练 TBGNN，并用验证集比较不同 GNN 模型是否加入楼层特征融合层。验证集包含 $66084$ 个预测结果，输出目标包括 $X$、$Y$ 两个方向的楼层位移 $\Delta_{wx}$、$\Delta_{wy}$，层间位移 $\delta_{wx}$、$\delta_{wy}$，以及一阶自振周期 $n_1$。

结果显示，加入楼层特征融合后，TBGNN 在验证集上的整体准确率由约 $84\%$ 提高到约 $92\%$。这说明楼层特征不是装饰性输入，而是高层建筑结构图学习中的关键组织信息。

![图 9 验证集回归性能](../../assets/public-safe/ref-tang2025-JBE/fig-09-regression-performance.jpg)
...
如果只使用平均绝对误差，模型可能在总体误差不大时仍然给出局部波动明显的位移曲线。对工程设计来说，这类不规则波动会影响构件评估和优化判断。

因此，论文把最大楼层响应和位移趋势纳入损失函数，让模型不仅追求点值接近，也学习楼层响应曲线应有的趋势。换句话说，模型训练目标从“整体误差更小”进一步靠近“曲线形态更符合工程判断”。

### 3. 楼层数比单纯结构高度更能影响模型外推

在楼层数敏感性分析中，论文比较了 $30$、$33$、$36$ 和 $39$ 层结构，并进一步比较相同高度但楼层数不同的结构。结果显示，当楼层数落在训练数据范围内时，各项指标准确率超过 $90\%$；当楼层数超过训练数据范围时，预测误差随超出程度增加而增大。

这提示我们，面向高层建筑的 GNN 代理模型不能只按结构高度理解训练范围。楼层数本身会改变结构图的信息传递深度和楼层特征组织方式，是后续训练和应用时需要优先审查的变量。

### 4. 模型能够响应风压变化和构件尺寸变化

论文以 $33$ 层结构为例，比较不同基本风压下的位移和层间位移预测。结果表明，TBGNN 可以较好学习荷载与结构响应之间的关系，对 $w_0$ 改变有明显响应。

![图 16 不同基本风压下的位移和层间位移](../../assets/public-safe/ref-tang2025-JBE/fig-16-wind-pressure-sensitivity.jpg)
...
![图 19 CAARC 建筑不同截面工况下预测值与真实值对比](../../assets/public-safe/ref-tang2025-JBE/fig-19-caarc-comparison.jpg)

图 19 CAARC 建筑不同截面工况下预测值与真实值对比

图中实线为真实值、虚线为预测值，不同颜色对应不同构件尺寸情景；该验证用于检查 TBGNN-TL 对同一拓扑下构件尺寸变化的响应预测能力。

### 5. 在论文测试条件下，参数化建模结合 TBGNN 显著节约分析时间

论文比较了三类流程：人工修改模型加 FEA、参数化修改模型加 FEA、参数化修改模型加 TBGNN 代理模型。在测试环境下，单次修改与分析总时间分别约为 $3\,\mathrm{min}\,13\,\mathrm{s}$、$1\,\mathrm{min}\,40\,\mathrm{s}$ 和 $17.33\,\mathrm{s}$。论文据此报告，相比传统分析流程，参数化建模结合 TBGNN 代理模型可节约约 $90\%$ 的时间。
