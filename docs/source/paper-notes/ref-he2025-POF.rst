...
这篇论文研究一种在 TLD 水箱内布置植入式立柱的减振方案，关注自由液面非线性、能量耗散和高层建筑风致响应控制。 它重要，因为高层建筑舒适度控制不仅取决于水箱质量和调谐频率，也取决于内部液体如何破碎、卷起、绕流并耗散能量。 读者可以带走的结论是：植入式立柱既是结构支撑构件，也是可设计的耗能构件，需要与流固耦合模型和调谐参数一起评估。

关键数字 / 关键结论卡
---------------------

- 双向耦合模型对结构位移和液体晃荡响应的平均相对误差分别为 :math:`5.26\%` 和 :math:`13.91\%`，计算时间由 :math:`5218\,\mathrm{s}` 降至 :math:`4632\,\mathrm{s}`。
- 在单自由度框架试验条件下，引入内部立柱后，结构顶部最大位移响应降低 :math:`85.2\%`。
- CAARC 建筑算例显示，立柱阻塞率和调谐比共同决定减振效率；在约 :math:`65\%` 的调谐范围内，植入式立柱 TLD 仍表现出一定鲁棒性。
...
.. figure:: ../../../wechat/assets/public-safe/ref-he2025-POF/fig-14-pole-flow-mechanism.jpg
   :alt: 论文图 14 自由液面速度云图说明植入式立柱对液体振荡的影响
   :align: center
   :width: 100%
   :class: paper-note-figure
...
**针对问题 1，没有立柱时，随着液深比 :math:`h/L` 增大，液体振荡会从 hard spring 特征转向 soft spring 特征，非线性特征也会发生变化。** 布置立柱后，液体晃荡行为明显改变：随着阻塞率 :math:`H` 增大，液体晃荡能量降低，自由液面上升受到限制，振荡响应减小。
...
**针对问题 1，激励幅值 :math:`A` 反映外部输入能量大小。** 论文结果显示，随着 :math:`A` 增大，波高和晃荡力峰值显著上升，振荡能量逐步积累，波形开始叠加，自由液面可能发生破碎；同时，晃荡周期减小、共振响应滞后，表现出 hard spring 效应。

.. figure:: ../../../wechat/assets/public-safe/ref-he2025-POF/fig-17-large-amplitude-nonlinear-sloshing.jpg
   :alt: 论文图 17 大幅激励下的非线性振荡特征
   :align: center
   :width: 100%
   :class: paper-note-figure
...
.. figure:: ../../../wechat/assets/public-safe/ref-he2025-POF/fig-18-energy-dissipation-excitation-amplitude.png
   :alt: 论文图 18 不同激励幅值下植入式立柱 TLD 的能量耗散效率
   :align: center
   :width: 100%
   :class: paper-note-figure
...
**针对问题 3，论文用单自由度框架结构和 TLD 控制试验对双向耦合数值模型进行验证。** 结构位移响应和液体晃荡响应与试验数据吻合，平均相对误差分别为 :math:`5.26\%` 和 :math:`13.91\%`。与标准流固耦合方法相比，该模型计算时间从 :math:`5218\,\mathrm{s}` 降至 :math:`4632\,\mathrm{s}`。
...
**针对问题 2，论文进一步以 CAARC 高层建筑模型为对象，研究 :math:`10` 年重现期风荷载下的风致振动控制。** 建筑高度为 :math:`182.88\,\mathrm{m}`，平面长度 :math:`45.72\,\mathrm{m}`、宽度 :math:`30.48\,\mathrm{m}`，并采用风洞测压试验数据作为风荷载输入。

.. figure:: ../../../wechat/assets/public-safe/ref-he2025-POF/fig-24-caarc-wind-tunnel-model.jpg
   :alt: 论文图 24 CAARC 模型测压试验示意
   :align: center
   :width: 100%
   :class: paper-note-figure
...
[68] :student-first-author:`He Xin`; **Li Chao**\*; Chen Lingwei; Hu Gang; Ou Jinping, Numerical investigation of nonlinear sloshing features and vibration mitigation efficiency of the implanted pole tuned liquid damper[J]. **Physics of Fluids**, 2025, 37(10): 103314. https://doi.org/10.1063/5.0293483.
