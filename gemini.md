...
| 2026-06-11 | 微信草稿 API 的 `content_source_url` 默认使用当前论文对应的 RTD 论文精解页：`https://woeai.readthedocs.io/zh-cn/latest/paper-notes/<publication_ref>.html`；只有用户针对某篇文章明确选择其他阅读原文目标或明确要求留空时，才在发布说明 front matter 写入 `wechat_content_source_url` 覆盖默认值。 | WOEAI 公众号自动化发布流程 | 对话确认 | 更新 |
...
| 2026-06-10 | WOEAI 公众号正文中的展示公式应在独立公式块中视觉居中，保留 `data-formula` 元信息并允许宽公式横向滚动。微信底部“阅读原文”由 API `content_source_url` 控制，不是正文“延伸阅读”小节；默认使用当前论文对应的 RTD 论文精解页，只有用户明确指定其他目标或留空时才在发布说明 front matter 中写入 `wechat_content_source_url` 覆盖默认值并提交。 | WOEAI 公众号论文文章 | 对话确认 | 更新 |
| 2026-06-10 | WOEAI 公众号论文文章仍以 `wechat/articles/draft-public-safe/*.md` 作为微信导读正文母版；微信后台预览若产生正文修改，必须先反向录回 Markdown，再重新生成微信草稿。2026-06-17 后，RTD 论文精解不再由公众号正文母版派生，而由 `docs/source/paper-notes/<publication_ref>.rst` 承载独立的全文精解。 | WOEAI 公众号论文文章 | 对话确认 | 更新 |
| 2026-06-11 | `wechat/tools/markdown_to_rtd.py` 只保留为历史公众号导读页维护工具，不作为新 RTD 论文精解全文页的默认生成入口。历史转换页的 RTD 渠道适配包括：页标题与“相关论文精解”链接文字去掉“数值风洞/结构抗风/漂浮风电 \|”分类前缀（仅 RTD 端）；识别 “点击阅读原文” 锚点并跳过微信固定结尾块；正文后追加“完整引用”小节（取自 `docs/source/Publications.rst` 对应锚点条目并截到 DOI 为止，不含影响因子与分区，附学术成果页锚点链接），再接内部“相关论文精解”导航。 | WOEAI 公众号论文文章 | 对话确认 | 更新 |
| 2026-06-17 | RTD 论文页面的公开标签统一为“论文精解”，仍放在 `docs/source/paper-notes/<publication_ref>.rst` 以保持既有 RTD/微信 `paper-notes/<publication_ref>.html` 链接稳定；长期方向是所有期刊论文进入论文精解流程。论文精解 RST 不再由公众号正文派生，而应按已批准可公开使用的 PDF/作者稿等论文源文件忠实保留章节、公式、图表、引用与参考文献；精简版微信公众号文章只是短导读，可在 RTD 页顶部链接，未发布时可预留。给撰写 agent 的简短提示词见 `docs/agents/paper-deep-dive-rst.md`。 | WOEAI RTD 论文精解 | 对话确认 | 更新 |
| 2026-06-17 | RTD 论文精解 RST 顶部必须在“精简版微信公众号文章：...”行之后立即放置对应公众号封面图，使用 `wechat/assets/public-safe/<publication_ref>/cover-wechat...` 的相对 `.. image::` 路径；论文正文尾部必须保留原论文附录（若有），但公开 RTD 论文精解不保留 `CRediT 作者贡献声明`、`利益冲突声明`、`数据可用性`、`致谢`、`补充材料` 等出版/声明性尾注。无附录时 `结论` 后直接进入 `参考文献`；有附录时顺序为 `结论`、`附录...`、`参考文献`。 | WOEAI RTD 论文精解 | 对话纠正 | 新增 |
| 2026-06-17 | 学术成果页面中带论文精解链接的期刊论文条目，可点击链接文字必须包含发表年份和中文精解标题，显示为 `YYYY \| 中文精解标题`；在 RST 中应写作 `[N] :doc:\`YYYY \| 中文精解标题 <paper-notes/<publication_ref>>\` ...`。Publication Number 不属于链接，作者、英文题名、期刊、DOI 和指标也不属于链接；生成脚本和手工维护都应保持该链接范围。 | WOEAI 学术成果页面 | 对话确认 | 新增 |
| 2026-06-17 | 学术成果页面左侧 RTD 侧边栏中，论文精解页面不得作为 Academic Outputs 的扁平直接子列表；应把对应 hidden toctree 写在匹配的研究大类与子方向标题下，使层级呈现为 `Academic Outputs -> Research Family -> Subdirection -> Paper Deep-Dive Page`。`_paper-notes-fragment.rst` 仅作为防止旧顶层 toctree 回归的空占位。 | WOEAI 学术成果页面 / RTD 论文精解 | 对话确认 | 新增 |
| 2026-06-17 | 学术成果的按年份倒序浏览页必须使用 `docs/source/PublicationsByYear.rst` 与公开 URL `PublicationsByYear.html`；不得继续使用旧的 by-research 文件名表达年份页，也不保留旧 URL 兼容跳转页。所有活跃源码、测试和站点正文链接都应指向 `PublicationsByYear`。 | WOEAI 学术成果页面 | 对话确认 | 新增 |
| 2026-06-17 | 学术成果相关页面左侧 RTD 侧边栏只承载真实浏览层级；`期刊论文 Journal Papers` 是正文说明标签，应使用 RST `.. rubric::`，不得作为 section 标题出现在侧边栏中。论文精解页侧边栏应保持 `Academic Outputs -> Research Family -> Subdirection -> Paper Deep-Dive Page`，中间不插入 `Journal Papers` 导航节点。 | WOEAI 学术成果页面 / RTD 论文精解 | 对话检查 | 新增 |
...
| 2026-06-17 | WOEAI RTD 论文精解 RST（`docs/source/paper-notes/*.rst`）的公式编号禁止使用 `\tag{N}`（MathJax 在 `aligned`/`cases`/`bmatrix` 等多行环境内会报 `\tag not allowed in split environment`）；统一改用 `\qquad (N)` 作为公式右侧手动编号。`scripts/check-public-safe-content.py` 已自动检测 `\tag{` 并拦截。 | WOEAI RTD 论文精解 | 实测确认 | 新增 |
