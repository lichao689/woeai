...
研究表明，经过迁移学习和本地数据微调后，RS-Mamba 能够较完整地提取 GF-7 影像中的建筑轮廓。论文报告的独立测试结果中，建筑识别的 Precision、Recall、F1-score 和 IoU 分别达到 :math:`0.9602`、:math:`0.9166`、:math:`0.9379` 和 :math:`0.9178`。
...
论文使用东莞理工学院区域的 UAV-LiDAR 数据作为地面真值，对 DSM 和建筑高度估计进行验证。结果显示，预测 DSM 与参考 DSM 的建筑高度具有较好一致性，量化指标达到 :math:`R^2 = 0.91`、:math:`\mathrm{MAE} = 2.72\,\mathrm{m}` 和 :math:`\mathrm{RMSE} = 4.09\,\mathrm{m}`。
...
.. figure:: ../../../wechat/assets/public-safe/ref-zhao2026-BE/fig-14-dsm-comparison.png
   :alt: 图 14 不同 DSM 生成与建筑高度估计算法的比较
   :align: center
   :width: 100%
   :class: paper-note-figure
