...
这篇论文研究内置矩形立柱如何改变矩形液舱和 TLD 的晃荡频率、阻尼性能与非线性自由液面。 它重要，因为内部立柱不是越多越好，也不是简单挡水；它会通过附加质量、尾流、剪切和自由液面破碎共同改变耗能路径。 读者可以带走的结论是：把 CFD 参数数据库压缩成等效机械模型，可以让 TLD 初步设计更快比较立柱位置、数量、阻塞率和激励幅值。

关键数字 / 关键结论卡
---------------------

- 传统纯水 TLD 的典型阻尼比量级约为 :math:`0.5\%`，这正是需要内部阻挡和耗能构造的原因。
- 等效机械模型对晃荡频率的拟合结果达到 :math:`R^2=0.984`、:math:`r=0.998`；对阻尼比的拟合结果达到 :math:`R^2=0.872`、:math:`r=0.934`。
...
**针对问题 1，随着立柱阻塞率 :math:`\Theta` 增大，周围液体被迫参与运动，附加质量效应增强，TLD 内部液体固有频率降低。** 与此同时，立柱会阻断液体能量传递路径，削弱自由液面上升和整体晃荡幅值。
...
**针对问题 2，激励幅值 :math:`A` 代表外部输入能量大小。** 小幅激励下，自由液面较平顺，流速较低，立柱边缘涡强度有限；随着 :math:`A` 增大，液体运动加剧，自由液面会出现卷起、破碎和更强的非线性波形，流速升高也会强化立柱边缘的涡脱落。
...
**针对问题 3，论文使用 PSO 从参数数据库中拟合等效机械模型。** 晃荡频率模型的拟合结果为 :math:`Q_1=-0.265`、:math:`Q_2=2.645`，决定系数 :math:`R^2=0.984`，相关系数 :math:`r=0.998`；阻尼比模型的拟合结果为 :math:`R^2=0.872`、:math:`r=0.934`。这说明，模型能够较好描述带内置矩形立柱 TLD 的频率与阻尼变化趋势。
...
.. figure:: ../../../wechat/assets/public-safe/ref-he2026-OE/fig-19-equivalent-model-validation.png
   :alt: 论文图 19 不同激励幅值下液体晃荡频率和阻尼比变化
   :align: center
   :width: 100%
   :class: paper-note-figure
...
[73] :student-first-author:`He Xin`; **Li Chao**\*; Chen Lingwei; Hu Gang; Ou Jinping, Numerical research on the impact of built-in rectangular poles on the dynamic characteristics of rectangular liquid tanks[J]. **Ocean Engineering**, 2026, 353: 124728. https://doi.org/10.1016/j.oceaneng.2026.124728.
