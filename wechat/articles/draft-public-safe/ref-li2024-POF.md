...
我们在这篇发表于 **Physics of Fluids** 的论文中提出矢量势随机流生成方法，即 vector potential random flow generation (VPRFG) method。它先生成矢量势场，再通过旋度得到脉动速度场，使合成湍流天然满足 $\nabla \cdot \mathbf{u}=0$，同时可以按目标能谱和 Taylor 冻结假设控制湍流统计特征。
...
这篇论文研究 VPRFG：先生成矢量势场，再通过旋度得到天然无散的湍动速度场。
它重要，因为 LES 入流如果只匹配目标能谱却不满足不可压缩流动的无散条件，后续压力-速度耦合可能会改变原本设定的湍流结构。
读者可以带走的结论是：湍动入流生成应把目标谱、无散性和 Taylor 冻结假设放进同一个数学构造，而不是靠后处理补救。

## 关键数字 / 关键结论卡

- VPRFG 不直接随机生成速度，而是通过 $\mathbf{u}=\nabla\times\boldsymbol{\psi}$ 让速度场天然满足无散条件。
...
![论文图 3 VPRFG 方法流程图](../../assets/public-safe/ref-li2024-POF/fig3-vprfg-flowchart.png)
...
为了让合成湍流在入口边界中按平均流输运，论文还把频率 $f_n$ 与波数第一分量 $k_{1,n}$ 联系起来，并得到 Taylor 冻结假设下的平移关系：
...
**针对问题 1，论文从三维空间互谱密度出发证明：当谱段数 $N$ 足够大时，VPRFG 计算得到的能谱 $E_{\mathrm{C}}(\kappa)$ 可以逼近目标能谱 $E_{\mathrm{T}}(\kappa)$。**
...
![论文图 4 以 von Karman 能谱为目标生成湍流的能谱](../../assets/public-safe/ref-li2024-POF/fig4-von-karman-energy-spectrum.jpg)
...
**针对问题 3，为了检验方法在 LES 初始场中的表现，论文使用 Comte-Bellot and Corrsin 实验能谱作为初始条件，开展均匀各向同性时间衰减盒湍流模拟。**计算域边长为 $0.2\pi\,\mathrm{m}$，最小波数为 $10\,\mathrm{m}^{-1}$，网格分辨率包括 $128^3$ 和 $256^3$，并与实验时刻 $U_0t/M=42$、$98$、$171$ 进行对比。

![论文图 7 初始时刻不同网格的 Q 准则等值面](../../assets/public-safe/ref-li2024-POF/fig7-q-criterion-isosurfaces.jpg)
...
![论文图 9 衰减盒湍流能谱](../../assets/public-safe/ref-li2024-POF/fig9-decaying-box-energy-spectra.jpg)
