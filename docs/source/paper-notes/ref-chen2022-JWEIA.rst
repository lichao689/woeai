...
.. math::
   ...
   u_i(\mathbf{x},t)=\sum_{n=1}^{N} p_{i,n}\sin(\mathbf{k}_n\cdot\mathbf{x}+2\pi f_n t+\varphi_n).\tag{2}
...
.. math::
   ...
   p_{i,n}=\mathrm{sign}(r_{i,n})\sqrt{2S_i^t(f_n)\Delta f}.\tag{3}

频率为：

.. math::

   f_n=\frac{2n-1}{2}\Delta f=\frac{(2n-1)f_{max}}{2N-1}.\tag{4}
...
.. math::
   ...
   \sqrt{S_i^t(f_n)S_j^t(f_n)}\Delta f}{\sigma_i\sigma_j}.\tag{7}

其中，标准差为：

.. math::

   \sigma_i=\sqrt{\sum_{n=1}^{N}S_i^t(f_n)\Delta f}.\tag{8}

由于 :math:`\mathrm{sign}(r_{i,n})\mathrm{sign}(r_{j,n})` 只可能取 :math:`-1`、0 或 1，最大计算互相关系数为：

.. math::

   \rho_{ij,C}^{max}=\frac{\sum_{n=1}^{N}\sqrt{S_i^t(f_n)S_j^t(f_n)}\Delta f}{\sigma_i\sigma_j}.\tag{9}

尺度因子为：

.. math::

   \xi_{ij}=\frac{\rho_{ij,T}}{\rho_{ij,C}^{max}}.\tag{10}

令随机变量服从均匀分布：

.. math::

   r_{i,n}\sim U(d_i-1,d_i),\qquad d_i\in[0,1].\tag{11}
...
.. math::
   ...
   =1+4d_id_j-2d_i-2d_j.\tag{14}

当 :math:`N\to\infty` 时：

.. math::

   \rho_{ij,C}=\xi_{ij}\rho_{ij,C}^{max}=\rho_{ij,T}.\tag{15}
...
.. math::
   ...
   k_{1,n}=-\frac{2\pi f_n}{U_{avg}}.\tag{17}
...
.. math::
   ...
   =E\left[u_i(\mathbf{x},t)u_i(\mathbf{x},t-\tau)\right]=R_i^t(-\tau)=R_i^t(\tau).\tag{19}
...
.. math::
   ...
   =\frac{U_{avg}}{2\pi}G_i^t(f).\tag{20}

一侧谱形式为：

.. math::

   S_i^x(k_1)=\frac{U_{avg}}{2\pi}S_i^t(f).\tag{21}
...
.. math::
   ...
   G_{u,T}^{y}(k_2)=\frac{(I_{u,T}U_{avg,T})^2}{2\pi}\int_{-\infty}^{\infty}\rho_{u,T}^{y}(r)\exp(-jk_2r)dr.\tag{23}
...
.. math::
   ...
   \left[\delta(k_2-k_{2,n})+\delta(k_2+k_{2,n})\right].\tag{24}
...
.. math::
   ...
   \lim_{N\to\infty}G_{u,C}^{y}(k_2)\approx G_{u,T}^{y}(k_2).\tag{30}

论文采用 Hemon and Santi 提出的空间相关表达：

.. math::

   Sc_{ij}=\sum_l\sqrt{S_{u,y_i}^{t}(f_l)S_{u,y_j}^{t}(f_l)}\,Coh_u^y(f_l)\Delta f.\tag{31}

.. math::

   Coh_u^y(f_l)=\exp\left(-\frac{C_u^y|y_i-y_j|f_l}{U_{avg}}\right)\Delta f.\tag{32}
...
.. math::
   ...
   \sum_{i=1}^{3}\frac{\partial u_i}{\partial x_i}=0.\tag{33}

若目标为生成均匀湍流场，式 (2) 中参数在不同空间点一致。代入式 (33)：

.. math::

   \sum_{n=1}^{N}\left[\left(\sum_{i=1}^{3}p_{i,n}k_{i,n}\right)\cos(\mathbf{k}_n\cdot\mathbf{x}+2\pi f_nt+\varphi_n)\right]=0.\tag{34}

令 :math:`\sum_{i=1}^{3}p_{i,n}k_{i,n}=0`，并结合式 (3)，得到：

.. math::

   \sum_{i=1}^{3}\mathrm{sign}(r_{i,n})\sqrt{S_i^t(f_n)}k_{i,n}=0.\tag{35}
...
.. math::
   ...
   C_p=\frac{p-p_{ref}}{0.5\rho U_H^2}.\tag{37}
...
.. math::
   ...
   u_i(\mathbf{x},t)=\sum_{m=1}^{M}\sum_{n=1}^{N}\left[p_i^{m,n}\cos(\tilde{\mathbf{k}}^{m,n}\cdot\tilde{\mathbf{x}}+\omega_{m,n}t)+q_i^{m,n}\sin(\tilde{\mathbf{k}}^{m,n}\cdot\tilde{\mathbf{x}}+\omega_{m,n}t)\right].\tag{A1}

.. math::

   p_i^{m,n}=\mathrm{sign}(r_i^{m,n})\sqrt{\frac{4}{N}S_i^{|k|}(k_m)\frac{(r_i^{m,n})^2}{1+(r_i^{m,n})^2}}.\tag{A2}

.. math::

   q_i^{m,n}=\mathrm{sign}(r_i^{m,n})\sqrt{\frac{4}{N}S_i^{|k|}(k_m)\frac{1}{1+(r_i^{m,n})^2}}.\tag{A3}

.. math::

   \tilde{\mathbf{x}}=\frac{\mathbf{x}}{L_s}.\tag{A4}

.. math::

   \mathbf{k}^{m,n}\cdot\mathbf{p}^{m,n}=0.\tag{A5}

.. math::

   \mathbf{k}^{m,n}\cdot\mathbf{q}^{m,n}=0.\tag{A6}

.. math::

   \tilde{\mathbf{k}}^{m,n}=\frac{\mathbf{k}^{m,n}}{k_0},\qquad |\mathbf{k}^{m,n}|=k_m.\tag{A7}

.. math::

   \omega_{m,n}\in N(0,2\pi f_m),\qquad f_m=k_mU_{avg}.\tag{A8}
...
.. math::
   ...
   u_i(\mathbf{x},t)=\sum_{m=1}^{M}\sum_{n=1}^{N}\left[p_i^{m,n}\cos\left(\tilde{\mathbf{k}}^{m,n}\cdot\tilde{\mathbf{x}}+\omega_{m,n}\frac{t}{\tau_0}\right)+q_i^{m,n}\sin\left(\tilde{\mathbf{k}}^{m,n}\cdot\tilde{\mathbf{x}}+\omega_{m,n}\frac{t}{\tau_0}\right)\right].\tag{B1}

.. math::

   p_i^{m,n}=\mathrm{sign}(r_i^{m,n})\sqrt{\frac{4c_i}{N}S_i^{|k|}(k_m)\Delta k_m\frac{(r_i^{m,n})^2}{1+(r_i^{m,n})^2}}.\tag{B2}

.. math::

   q_i^{m,n}=\mathrm{sign}(r_i^{m,n})\sqrt{\frac{4c_i}{N}S_i^{|k|}(k_m)\Delta k_m\frac{1}{1+(r_i^{m,n})^2}}.\tag{B3}
...
.. math::
   ...
   u_i(\mathbf{x},t)=\sum_{m=1}^{M}\sum_{n=1}^{N}\left[p_i^{m,n}\cos(\mathbf{k}^{m,n}\cdot\tilde{\mathbf{x}}^m+2\pi f_{m,n}t)+q_i^{m,n}\sin(\mathbf{k}^{m,n}\cdot\tilde{\mathbf{x}}^m+2\pi f_{m,n}t)\right].\tag{C1}

.. math::

   p_i^{m,n}=\mathrm{sign}(r_i^{m,n})\sqrt{\frac{2}{N}S_i^t(f_m)\Delta f\frac{(r_i^{m,n})^2}{1+(r_i^{m,n})^2}}.\tag{C2}

.. math::

   q_i^{m,n}=\mathrm{sign}(r_i^{m,n})\sqrt{\frac{2}{N}S_i^t(f_m)\Delta f\frac{1}{1+(r_i^{m,n})^2}}.\tag{C3}

.. math::

   \tilde{x}_{j}^{m}=\frac{x_j}{L_{j}^{m}}.\tag{C4}
...
.. math::
   ...
   \end{bmatrix}
   =
   \begin{bmatrix}0\\0\\1\end{bmatrix}.\tag{C5}

.. math::

   f_{m,n}\in N(f_m,\Delta f).\tag{C6}

.. math::

   L_{j,m}=\frac{U_{avg}}{\gamma C_j f_m}.\tag{C7}
...
.. math::
   ...
   u_i(\mathbf{x},t)=\sum_{n=1}^{N}\sqrt{2S_i^t(f_n)\Delta f}\sin(\mathbf{k}_n\cdot\tilde{\mathbf{x}}_n+2\pi f_nt+\varphi_n).\tag{D1}

.. math::

   \tilde{x}_{j,n}=\frac{x_j}{L_{j,n}}.\tag{D2}
...
.. math::
   ...
   L_{j,n}=\frac{U_{avg}}{\gamma_jC_jf_n}.\tag{D6}
