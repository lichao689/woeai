...
CWR 方法的核心，是把反馈控制放入 weak recycling 的重缩放流程。论文首先在回收站收集瞬时速度，计算模拟湍流强度与目标湍流强度之间的误差，再构造控制系数 $\lambda$ 调整回收的纵向脉动速度，最后把更新后的入口速度重新施加到计算域中。这个过程循环进行，直到回收站处的湍流强度与目标值对齐。
